...
export default function MassFlowPage() {
  const [values, setValues] = useState<PressureValues>({
    bar: '', MPa: '', Pa: '', atm: '', psi: '', kgfPerM2: ''
  });

  // 轉換因子
  const conversionFactors: Record<PressureUnit, number> = {
    bar: 1, MPa: 0.1, Pa: 100000, atm: 0.986923,
    psi: 14.5038, kgfPerM2: 10197.2
  };
  ...
  const calculateAll = (value: string, unit: PressureUnit) => {
    ...
    // 先轉換為巴
    const valueInBar = unit === 'bar' ? numValue : numValue / conversionFactors[unit];

    // 從巴轉換為其他單位
    (Object.keys(conversionFactors) as PressureUnit[]).forEach(targetUnit => {
      if (targetUnit !== unit) {
        newValues[targetUnit] = (valueInBar * conversionFactors[targetUnit]).toFixed(6);
      }
    });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>, unit: PressureUnit) => {
