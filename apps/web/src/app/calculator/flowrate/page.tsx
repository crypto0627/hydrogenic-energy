...
export default function FlowRatePage() {
  ...
  const calculateAll = (value: string, unit: FlowRateUnit) => {
    ...
    const timeUnit = unit
      .replace(/^(liter|cubicMeter)Per/, '')
      .toLowerCase() as FlowRateTimeUnit
    ...
    Object.keys(timeConversionFactors).forEach((time) => {
      ...
      // 公升單位
      const literKey =
        `literPer${timeKey.charAt(0).toUpperCase() + timeKey.slice(1)}` as FlowRateUnit
      if (literKey !== unit) {
        newValues[literKey] = (
      ...
      // 立方米單位
      const cubicMeterKey =
        `cubicMeterPer${timeKey.charAt(0).toUpperCase() + timeKey.slice(1)}` as FlowRateUnit
